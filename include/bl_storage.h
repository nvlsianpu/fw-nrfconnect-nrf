...
NRFX_STATIC_INLINE uint16_t bl_storage_otp_halfword_read(uint32_t address)
{
	uint16_t halfword;
#ifdef CONFIG_NRFX_NVMC
	halfword = nrfx_nvmc_otp_halfword_read(aligned_address);
#elif defined(CONFIG_NRFX_RRAMC)
...
NRFX_STATIC_INLINE int read_life_cycle_state(enum lcs *lcs)
{
	...
	uint32_t secure = nrfx_rramc_otp_word_read(
			index_from_address((uint32_t) &BL_STORAGE->lcs.secure));
	uint32_t decommissioned = nrfx_rramc_otp_word_read(
			index_from_address((uint32_t) &BL_STORAGE->lcs.decommissioned));
#endif
