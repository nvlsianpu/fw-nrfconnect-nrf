 ...
 *
 * Custom CDC ACM serial init that registers all CDC ACM instances from
 * devicetree (e.g. cdc_acm_0 for mcumgr, cdc_acm_1 for log). Use with
 * CONFIG_FW_LOADER_CUSTOM_USB=y and CONFIG_CDC_ACM_SERIAL_INITIALIZE_AT_BOOT=n.
 */
...
static int register_cdc_acm_instances(struct usbd_context *const uds_ctx,
				     const enum usbd_speed speed)
{
	...
	for (uint8_t i = 0; i < CDC_ACM_INSTANCE_COUNT; i++) {
		char name[sizeof("cdc_acm_255")];

		snprintf(name, sizeof(name), "cdc_acm_%u", i);
		err = usbd_register_class(uds_ctx, name, speed, 1);
		if (err) {
			LOG_ERR("Failed to register class %s", name);
			return err;
		}
	}
...
static int cdc_acm_serial_init_device(void)
{
	...
	if (USBD_SUPPORTS_HIGH_SPEED &&
		...
		err = register_cdc_acm_instances(&cdc_acm_serial, USBD_SPEED_HS);
		if (err) {
			return err;
		}
	}

	err = register_cdc_acm_instances(&cdc_acm_serial, USBD_SPEED_FS);
	if (err) {
		return err;
	}
